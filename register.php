...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register'])) {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Store password as plain text instead of hashing it
            $plain_password = $password; // Store the password in plain text
            
            // Insert new user with plain text password
            $sql = "INSERT INTO users (first_name, last_name, email, password, phone, role) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $first_name, $last_name, $email, $plain_password, $phone, $role);
